...
文書版: 1.5
基準日: 2026-09-25（配布・検証状態は RELEASE_DELIVERY_2026-09-25_JA.md 参照）
対象アプリ版: 1.0.0
対象アプリソース: `72c32ad`（継続グループ・強化チャット・通知基盤、場所保存修正を含む）。後続の配信補助スクリプト・文書変更はネイティブアプリの機能変更ではない。
正式名称: TSUDOWA（ツドワ）
提供予定地域: 日本
運営者: 玉木 秀杷（タマキ シュウワ）
...
- 日時・説明・場所更新は保存成功後に状態を反映し、失敗時は入力を保持する。場所修正を含むソースはiOS 16 / Android 13にビルド済み。iOS 16はTestFlight外部テスト審査承認済み。実機による受け入れ確認は別途必要。
...
2. 日時・説明・場所はmanager checkとawait、失敗時の入力保持を実装済み。通常参加者が詳細から開いた場合は保存を許可しない。場所修正の追加6項目を含むDB試験は5ファイル92項目がローカル・CIともに成功。本番データを用いる実機試験とは区別する。Docker起動障害は一時復旧済みだが再起動時の再発を確認しており、恒久解決とはしない。
