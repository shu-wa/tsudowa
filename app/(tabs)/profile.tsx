  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.cloudStatus}><Ionicons name={isConfigured ? 'cloud-outline' : 'phone-portrait-outline'} size={21} color={palette.primary} /><View style={styles.cloudCopy}><Text style={styles.cloudTitle}>{isConfigured ? 'クラウド保存を使用' : 'この端末に保存'}</Text><Text style={styles.cloudText}>{isConfigured ? user?.email ?? 'ログイン済み' : 'イベント情報はこの端末に保存されています'}</Text></View></View>

        <Text style={styles.sectionTitle}>設定</Text>
        <View style={styles.settings}>
          <View style={styles.row}>
            <View style={styles.rowIcon}><Ionicons name="notifications-outline" size={20} color={palette.primary} /></View><View style={styles.rowCopy}><Text style={styles.rowLabel}>通知</Text><Text style={styles.rowSub}>リマインダーと新着情報を受け取る</Text></View><Switch value={settings.notificationsEnabled} onValueChange={changeNotifications} disabled={notificationUpdating} trackColor={{ false: '#BFC2BE', true: palette.primary }} thumbColor={palette.surface} />
          </View>
          <TouchableOpacity style={styles.row} onPress={() => router.push('/notification-settings' as Href)}><View style={styles.rowIcon}><Ionicons name="options-outline" size={20} color={palette.primary} /></View><View style={styles.rowCopy}><Text style={styles.rowLabel}>通知の詳細設定</Text><Text style={styles.rowSub}>種類、時間帯、イベントごとに選択</Text></View><Ionicons name="chevron-forward" size={18} color={palette.muted} /></TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  profileCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: palette.surface, borderRadius: 8, borderWidth: 1, borderColor: palette.line, padding: 17 }, profileCopy: { flex: 1, marginLeft: 14 }, name: { color: palette.ink, fontSize: 18, fontWeight: '800', marginBottom: 4 }, handle: { color: palette.muted, fontSize: 12 }, edit: { width: 36, height: 36, borderRadius: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.primarySoft },
  stats: { flexDirection: 'row', backgroundColor: palette.primary, borderRadius: 8, paddingVertical: 18, marginTop: 12, marginBottom: 28 }, stat: { flex: 1, alignItems: 'center' }, statNumber: { color: palette.surface, fontSize: 20, fontWeight: '800', marginBottom: 3 }, statLabel: { color: '#D8DDD9', fontSize: 12 }, divider: { width: StyleSheet.hairlineWidth, backgroundColor: 'rgba(255,255,255,0.25)' },
  cloudStatus: { flexDirection: 'row', alignItems: 'center', borderLeftWidth: 3, borderLeftColor: palette.primary, padding: 14, marginTop: -16, marginBottom: 25 }, cloudCopy: { flex: 1, marginLeft: 10 }, cloudTitle: { color: palette.ink, fontSize: 13, fontWeight: '900' }, cloudText: { color: palette.muted, fontSize: 12, marginTop: 3 },
  sectionTitle: { color: palette.ink, fontSize: 19, fontWeight: '800', marginBottom: 12 }, settings: { backgroundColor: palette.surface, borderRadius: 8, borderWidth: 1, borderColor: palette.line, overflow: 'hidden' }, row: { minHeight: 72, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: palette.line }, rowIcon: { width: 38, height: 38, borderRadius: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.primarySoft }, rowCopy: { flex: 1, marginLeft: 12 }, rowLabel: { color: palette.ink, fontSize: 14, fontWeight: '700', marginBottom: 3 }, rowSub: { color: palette.muted, fontSize: 12, lineHeight: 17 }, resetButton: { height: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 18 }, resetText: { color: palette.danger, fontSize: 13, fontWeight: '700', marginLeft: 6 }, version: { textAlign: 'center', color: palette.muted, fontSize: 12, marginTop: 10 },
});
