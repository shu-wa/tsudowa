...
export default function CollectionDetailScreen() {
  ...
  const { findEvent, profile, toggleCollectionPayment, deleteCollection } = useEvents();
  const { user } = useAuth();
  const event = findEvent(id);
  const collection = event?.collections.find((item) => item.id === collectionId);
  const canManageCollection = Boolean(event && !isEventArchived(event) && isEventHost(event, user?.id, profile.name));
  ...
  const confirmDelete = () => Alert.alert(
    'この集金項目を削除しますか？',
    '金額と全員の支払状態が削除され、元に戻せません。',
    [{ text: 'キャンセル', style: 'cancel' }, {
      text: '削除する',
      style: 'destructive',
      onPress: async () => {
        const error = await deleteCollection(event.id, collection.id);
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.hero, { backgroundColor: category.background }]}>
          ...
          {canManageCollection ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="集金項目を編集" style={styles.editButton} onPress={() => router.push({ pathname: '/event/[id]/collection/new', params: { id: event.id, collectionId: collection.id } })}><Ionicons name="pencil-outline" size={18} color={palette.primary} /></TouchableOpacity> : null}
        </View>
        ...
        <View style={styles.sectionHeader}><Text style={styles.sectionTitleInline}>対象メンバー</Text><Text style={styles.tapHint}>{canManageCollection ? 'タップで支払状態を変更' : '主催者のみ変更可能'}</Text></View>
        <View style={styles.members}>
          {collection.shares.map((share, index) => {
            ...
            return <TouchableOpacity accessibilityRole="button" accessibilityState={{ disabled: !canManageCollection }} key={share.participantId} style={[styles.memberRow, index === collection.shares.length - 1 && styles.memberRowLast]} onPress={() => togglePayment(share.participantId)} activeOpacity={canManageCollection ? 0.7 : 1} disabled={!canManageCollection}>
        ...
        <View style={styles.notice}><Ionicons name="information-circle-outline" size={18} color={palette.muted} /><Text style={styles.noticeText}>支払い方法は現金や各種送金サービスから自由に選べます。支払状態は主催者が確認して更新し、参加者全員へ共有されます。</Text></View>
        {canManageCollection ? <TouchableOpacity accessibilityRole="button" style={styles.deleteButton} onPress={confirmDelete}><Ionicons name="trash-outline" size={18} color={palette.danger} /><Text style={styles.deleteText}>この集金項目を削除</Text></TouchableOpacity> : null}
      </ScrollView>
    </SafeAreaView>
  );
}
