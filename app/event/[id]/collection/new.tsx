...
import { isEventHost } from '@/lib/event-permissions';
...
export default function NewCollectionScreen() {
  ...
  const { findEvent, addCollection, updateCollection, profile } = useEvents();
  const { user } = useAuth();
  const event = findEvent(id);
  const editingCollection = event?.collections.find((collection) => collection.id === collectionId);
  const editing = Boolean(collectionId);
  const initialSplitMode: SplitMethod | 'perPerson' = editingCollection?.autoAssignNewMembers ? 'perPerson' : editingCollection?.splitMethod ?? 'equal';
  const [title, setTitle] = useState(editingCollection?.title ?? '');
  const [category, setCategory] = useState<CollectionCategory>(editingCollection?.category ?? 'entry');
  const [amount, setAmount] = useState(editingCollection ? String(editingCollection.autoAssignNewMembers ? editingCollection.defaultShareAmount ?? 0 : editingCollection.totalAmount) : '');
  const [payerId, setPayerId] = useState(editingCollection?.paidByParticipantId ?? event?.participants[0]?.id ?? '');
  const [splitMode, setSplitMode] = useState<SplitMethod | 'perPerson'>(initialSplitMode);
  const [includedIds, setIncludedIds] = useState<string[]>(editingCollection?.autoAssignNewMembers
    ? event?.participants.map((person) => person.id) ?? []
    : editingCollection?.shares.map((share) => share.participantId) ?? event?.participants.map((person) => person.id) ?? []);
  const [customAmounts, setCustomAmounts] = useState<Record<string, string>>(() => Object.fromEntries(editingCollection?.shares.map((share) => [share.participantId, String(share.amount)]) ?? []));
  const [dueDate, setDueDate] = useState(editingCollection?.dueDate ?? '');
  const [dueDateOpen, setDueDateOpen] = useState(false);
  const [note, setNote] = useState(editingCollection?.note ?? '');
  ...
  if (editing && !editingCollection) return <SafeAreaView style={styles.empty}><Text>集金項目が見つかりません</Text></SafeAreaView>;
  if (!isEventHost(event, user?.id, profile.name)) return <SafeAreaView style={styles.empty}><Ionicons name="lock-closed-outline" size={32} color={palette.muted} /><Text style={styles.permissionTitle}>集金を編集できません</Text><Text style={styles.permissionText}>集金を追加・編集できるのは主催者だけです。</Text><TouchableOpacity onPress={() => router.back()}><Text style={styles.permissionBack}>戻る</Text></TouchableOpacity></SafeAreaView>;

  const toggleParticipant = (participantId: string) => {
    if (splitMode === 'perPerson') return;
    setIncludedIds((current) => current.includes(participantId) ? current.filter((item) => item !== participantId) : [...current, participantId]);
  };
  const selectPerPersonSplit = () => {
    setSplitMode('perPerson');
    setIncludedIds(event.participants.map((person) => person.id));
  };

  const submit = async () => {
    if (!title.trim()) return Alert.alert('集金名を入力してください');
    if (numericAmount <= 0) return Alert.alert(splitMode === 'perPerson' ? '1人あたりの金額を入力してください' : '合計金額を入力してください');
    if (!payerId) return Alert.alert('立替・支払った人を選択してください');
    if (includedIds.length === 0) return Alert.alert('集金対象者を1人以上選択してください');
    if (splitMode === 'custom' && customTotal !== numericAmount) {
      return Alert.alert('個別金額を確認してください', `個別金額の合計は ¥${customTotal.toLocaleString()} です。集金総額 ¥${numericAmount.toLocaleString()} と一致させてください。`);
    }
    const perPersonAmounts = Object.fromEntries(includedIds.map((participantId) => [participantId, numericAmount]));
    const input = {
      title: title.trim(), category, paidByParticipantId: payerId,
      totalAmount: splitMode === 'perPerson' ? numericAmount * includedIds.length : numericAmount,
      splitMethod: splitMode === 'perPerson' ? 'custom' as const : splitMode,
      participantIds: includedIds, dueDate: dueDate.trim() || undefined, note: note.trim() || undefined,
      customAmounts: splitMode === 'perPerson' ? perPersonAmounts : Object.fromEntries(Object.entries(customAmounts).map(([key, value]) => [key, Number(value) || 0])),
      autoAssignNewMembers: splitMode === 'perPerson',
      defaultShareAmount: splitMode === 'perPerson' ? numericAmount : undefined,
    };
    setSaving(true);
    const error = editingCollection
      ? await updateCollection(event.id, editingCollection.id, input)
      : await addCollection(event.id, input);
    setSaving(false);
    if (error) return Alert.alert(editing ? '集金を更新できませんでした' : '集金を追加できませんでした', error);
    router.back();
  };

  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <Stack.Screen options={{ title: editing ? '集金項目を編集' : '集金項目を追加' }} />
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.content} automaticallyAdjustKeyboardInsets keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} onScrollBeginDrag={Keyboard.dismiss} showsVerticalScrollIndicator={false}>
          <View style={styles.intro}><View style={styles.introIcon}><Ionicons name="receipt" size={22} color={palette.primary} /></View><View style={styles.introCopy}><Text style={styles.introTitle}>{editing ? '集金内容を編集' : '費用ごとに分けて管理'}</Text><Text style={styles.introText}>{editing ? '金額、期限、対象メンバーなどを変更できます' : '参加費、食事代、立替えなどを何件でも追加できます'}</Text></View></View>
          ...
          <FieldLabel label={splitMode === 'perPerson' ? '1人あたりの金額' : '集金総額'} required />
          <View style={styles.amountWrap}><Text style={styles.yen}>¥</Text><TextInput value={amount} onChangeText={setAmount} placeholder="0" placeholderTextColor="#9AA29D" style={styles.amountInput} keyboardType="number-pad" returnKeyType="done" onSubmitEditing={Keyboard.dismiss} blurOnSubmit /><Text style={styles.taxText}>{splitMode === 'perPerson' ? '1人分' : '税込'}</Text></View>
          ...
          <View style={styles.splitRow}>
            <TouchableOpacity style={[styles.splitButton, splitMode === 'equal' && styles.splitButtonActive]} onPress={() => setSplitMode('equal')}><Ionicons name="git-compare-outline" size={19} color={splitMode === 'equal' ? palette.surface : palette.primary} /><View><Text style={[styles.splitTitle, splitMode === 'equal' && styles.splitTitleActive]}>均等割り</Text><Text style={[styles.splitText, splitMode === 'equal' && styles.splitTextActive]}>対象者で自動計算</Text></View></TouchableOpacity>
            <TouchableOpacity style={[styles.splitButton, splitMode === 'custom' && styles.splitButtonActive]} onPress={() => setSplitMode('custom')}><Ionicons name="options-outline" size={19} color={splitMode === 'custom' ? palette.surface : palette.primary} /><View><Text style={[styles.splitTitle, splitMode === 'custom' && styles.splitTitleActive]}>個別指定</Text><Text style={[styles.splitText, splitMode === 'custom' && styles.splitTextActive]}>人ごとに金額を変更</Text></View></TouchableOpacity>
            <TouchableOpacity style={[styles.splitButton, splitMode === 'perPerson' && styles.splitButtonActive]} onPress={selectPerPersonSplit}><Ionicons name="person-add-outline" size={19} color={splitMode === 'perPerson' ? palette.surface : palette.primary} /><View><Text style={[styles.splitTitle, splitMode === 'perPerson' && styles.splitTitleActive]}>1人あたり</Text><Text style={[styles.splitText, splitMode === 'perPerson' && styles.splitTextActive]}>現在・新規の参加者全員</Text></View></TouchableOpacity>
          </View>

          <FieldLabel label="集金対象者" hint={splitMode === 'perPerson' ? '参加者全員が対象' : `${includedIds.length}/${event.participants.length}人を選択`} />
          <View style={styles.memberList}>
            {event.participants.map((person, index) => {
              ...
              return <View key={person.id} style={[styles.memberRow, index === event.participants.length - 1 && styles.memberRowLast]}>
                <TouchableOpacity style={styles.memberSelect} onPress={() => toggleParticipant(person.id)} disabled={splitMode === 'perPerson'}><View style={[styles.check, selected && styles.checkSelected]}>{selected && <Ionicons name="checkmark" size={15} color={palette.surface} />}</View><View style={[styles.memberAvatar, { backgroundColor: person.avatarColor }]}><Text style={styles.memberInitials}>{person.initials}</Text></View><View style={styles.memberCopy}><Text style={styles.memberName}>{person.name}</Text><Text style={styles.memberAttendance}>{splitMode === 'perPerson' ? '参加者全員の集金対象' : person.attendance}</Text></View></TouchableOpacity>
                {selected && <View style={styles.shareAmount}>{splitMode === 'equal' ? <Text style={styles.shareText}>¥{(equalAmount + (includedIds[0] === person.id ? equalRemainder : 0)).toLocaleString()}</Text> : splitMode === 'perPerson' ? <Text style={styles.shareText}>¥{numericAmount.toLocaleString()}</Text> : <><Text style={styles.shareYen}>¥</Text><TextInput value={customAmounts[person.id] ?? ''} onChangeText={(value) => setCustomAmounts((current) => ({ ...current, [person.id]: value }))} placeholder="0" placeholderTextColor="#9AA29D" style={styles.shareInput} keyboardType="number-pad" returnKeyType="done" onSubmitEditing={Keyboard.dismiss} blurOnSubmit /></>}</View>}
              </View>;
            })}
          </View>
          {splitMode === 'custom' && <View style={[styles.sumNotice, customTotal === numericAmount && numericAmount > 0 && styles.sumNoticeOk]}><Text style={styles.sumNoticeText}>個別金額の合計</Text><Text style={styles.sumNoticeAmount}>¥{customTotal.toLocaleString()} / ¥{numericAmount.toLocaleString()}</Text></View>}
          {splitMode === 'perPerson' && <View style={styles.sumNotice}><Text style={styles.sumNoticeText}>現在の合計</Text><Text style={styles.sumNoticeAmount}>¥{(numericAmount * includedIds.length).toLocaleString()}</Text></View>}
        ...
        <View style={styles.bottom}><KeyboardDismissBar /><TouchableOpacity style={[styles.submit, saving && styles.submitDisabled]} onPress={submit} disabled={saving}><Text style={styles.submitText}>{saving ? '保存中…' : editing ? '変更を保存する' : '集金項目を追加する'}</Text><Ionicons name={editing ? 'checkmark' : 'arrow-forward'} size={19} color={palette.surface} /></TouchableOpacity></View>
...
const styles = StyleSheet.create({
  ...
  splitRow: { gap: 8, marginBottom: 19 }, splitButton: { minHeight: 62, borderRadius: 17, borderWidth: 1, borderColor: palette.line, backgroundColor: palette.surface, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 10 }, splitButtonActive: { backgroundColor: palette.primary, borderColor: palette.primary }, splitTitle: { color: palette.ink, fontSize: 13, fontWeight: '800', marginBottom: 3 }, splitTitleActive: { color: palette.surface }, splitText: { color: palette.muted, fontSize: 11 }, splitTextActive: { color: '#C8D8CF' },
  ...
  bottom: { padding: 14, backgroundColor: palette.surface, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: palette.line }, submit: { height: 55, borderRadius: 18, backgroundColor: palette.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }, submitDisabled: { opacity: 0.55 }, submitText: { color: palette.surface, fontSize: 14, fontWeight: '800', marginRight: 8 },
});
