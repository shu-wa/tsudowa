...
import { defaultProfileHandleBody, normalizeProfileHandle, PROFILE_HANDLE_TAKEN_MESSAGE, validateProfileHandle } from '@/lib/profile-handle';
...
export default function OnboardingScreen() {
  ...
  // Authenticated users already passed the account screen. If their profile is
  // incomplete, take them straight to the missing registration fields instead
  // of showing the marketing welcome screen again.
  const [step, setStep] = useState(knownProfile ? 2 : user ? 1 : 0);
  const [name, setName] = useState(profile.name === '新しいメンバー' ? '' : profile.name);
  const [email, setEmail] = useState(user?.email ?? profile.email ?? '');
  const [handle, setHandle] = useState(() => defaultProfileHandleBody(user?.email ?? profile.email ?? ''));
  ...
  const [birthDateLimits] = useState(() => {
    ...
    const maximum = new Date(today.getFullYear() - 16, today.getMonth(), today.getDate(), 12);
    return { minimum: new Date(1900, 0, 1, 12), maximum, pickerDefault };
  });
  ...
  const finish = async () => {
    ...
    setSubmitting(true);
    const error = await completeOnboarding({ name, handle: normalizeProfileHandle(handle), email, dateOfBirth });
    setSubmitting(false);
    if (error === PROFILE_HANDLE_TAKEN_MESSAGE) {
      setStep(1);
      return Alert.alert('この表示IDは使用できません', error);
    }
    if (error) return Alert.alert('登録できませんでした', error);
    router.replace('/');
  };
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView contentContainerStyle={styles.content} automaticallyAdjustKeyboardInsets keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} showsVerticalScrollIndicator={false}>
          ...
          {step === 1 && <>
            <View style={styles.stepIcon}><Ionicons name="person-add-outline" size={28} color={palette.primary} /></View><Text style={styles.eyebrow}>アカウント設定</Text><Text style={styles.title}>最初の登録</Text><Text style={styles.lead}>イベント内で使う表示名と、本人確認に使う情報を登録します。</Text>
            <Input label="表示名" value={name} onChangeText={setName} placeholder="例：佐藤 美咲" autoFocus />
            <Input label="表示ID" hint="プロフィールには @付きで表示されます" value={handle} onChangeText={setHandle} placeholder="tamasyu0202" autoCapitalize="none" autoCorrect={false} maxLength={31} />
            <Input label="メールアドレス" value={email} onChangeText={setEmail} editable={!user?.email} placeholder="you@example.com" keyboardType="email-address" autoCapitalize="none" />
          ...
          {step === 2 && <>
            <View style={styles.stepIcon}><Ionicons name="shield-checkmark-outline" size={29} color={palette.primary} /></View><Text style={styles.eyebrow}>利用前の確認</Text><Text style={styles.title}>同意内容の確認</Text><Text style={styles.lead}>3つの文書をそれぞれ確認し、同意内容を選択してください。</Text>
...
function Input({ label, hint, ...props }: { label: string; hint?: string } & React.ComponentProps<typeof TextInput>) { return <View style={styles.inputGroup}><Text style={styles.inputLabel}>{label}</Text><TextInput {...props} style={styles.input} placeholderTextColor="#9AA39E" selectionColor={palette.primary} />{hint ? <Text style={styles.inputHint}>{hint}</Text> : null}</View>; }
function PrimaryButton({ label, onPress, loading = false }: { label: string; onPress: () => void; loading?: boolean }) { return <TouchableOpacity accessibilityRole="button" style={[styles.primaryButton, loading && styles.primaryButtonDisabled]} onPress={onPress} disabled={loading}>{loading ? <ActivityIndicator color={palette.surface} /> : <><Text style={styles.primaryText}>{label}</Text><Ionicons name="arrow-forward" size={19} color={palette.surface} /></>}</TouchableOpacity>; }
...
const styles = StyleSheet.create({
  ...
  logo: { width: 78, height: 78, borderRadius: 12, marginTop: 25, marginBottom: 17 }, brand: { color: palette.accent, fontSize: 12, fontWeight: '900', letterSpacing: 2 }, welcomeTitle: { color: palette.ink, fontSize: 31, lineHeight: 41, fontWeight: '900', marginTop: 8 }, welcomeLead: { color: palette.muted, fontSize: 14, lineHeight: 22, marginTop: 10 }, features: { marginVertical: 30 }, feature: { flexDirection: 'row', alignItems: 'center', paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: palette.line }, featureIcon: { width: 42, height: 42, borderRadius: 6, backgroundColor: palette.primarySoft, alignItems: 'center', justifyContent: 'center' }, featureCopy: { flex: 1, marginLeft: 12 }, featureTitle: { color: palette.ink, fontSize: 14, fontWeight: '900' }, featureText: { color: palette.muted, fontSize: 12, lineHeight: 17, marginTop: 3 }, ageNote: { color: palette.muted, fontSize: 12, textAlign: 'center', marginTop: 12 },
  stepIcon: { width: 54, height: 54, borderRadius: 8, backgroundColor: palette.primarySoft, alignItems: 'center', justifyContent: 'center', marginBottom: 18 }, eyebrow: { color: palette.accent, fontSize: 12, fontWeight: '900', letterSpacing: 1.6 }, title: { color: palette.ink, fontSize: 28, fontWeight: '900', marginTop: 5 }, lead: { color: palette.muted, fontSize: 12, lineHeight: 19, marginTop: 8, marginBottom: 24 }, inputGroup: { marginBottom: 15 }, inputLabel: { color: palette.ink, fontSize: 13, fontWeight: '800', marginBottom: 7 }, input: { height: 54, borderRadius: 8, backgroundColor: palette.surface, paddingHorizontal: 15, color: palette.ink, fontSize: 14, borderWidth: 1, borderColor: palette.line }, inputHint: { color: palette.muted, fontSize: 11, lineHeight: 16, marginTop: 5 }, notice: { flexDirection: 'row', borderLeftWidth: 3, borderLeftColor: palette.primary, backgroundColor: palette.primarySoft, padding: 13, marginBottom: 22 }, noticeText: { flex: 1, color: palette.primary, fontSize: 12, lineHeight: 15, marginLeft: 8 },
  primaryButton: { minHeight: 57, borderRadius: 8, backgroundColor: palette.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 10 }, primaryButtonDisabled: { opacity: 0.6 }, primaryText: { color: palette.surface, fontSize: 14, fontWeight: '900', marginRight: 9 }, back: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 17 }, backText: { color: palette.muted, fontSize: 13, fontWeight: '700', marginLeft: 5 },
  consentRow: { flexDirection: 'row', alignItems: 'center', borderRadius: 8, borderWidth: 1, borderColor: palette.line, backgroundColor: palette.surface, padding: 14, marginBottom: 10 }, checkbox: { width: 30, height: 30, borderRadius: 4, borderWidth: 2, borderColor: '#8B908C', alignItems: 'center', justifyContent: 'center' }, checkboxChecked: { backgroundColor: palette.primary, borderColor: palette.primary }, consentCopy: { flex: 1, marginLeft: 12 }, consentTitle: { color: palette.ink, fontSize: 14, lineHeight: 20, fontWeight: '800' }, required: { color: palette.accent, fontSize: 13 }, read: { color: palette.primary, fontSize: 12, fontWeight: '700', marginTop: 4 },
});
