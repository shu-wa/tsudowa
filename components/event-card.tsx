  ...
  return (
    <TouchableOpacity style={[styles.card, featured && styles.featured]} onPress={() => router.push(`/event/${event.id}`)} activeOpacity={0.88}>
      <View style={[styles.cover, { backgroundColor: event.coverColor }]}>
        ...
        {!event.coverImageUri && <View style={styles.coverArt}>
        ...
        <View style={styles.footer}>
          <View style={styles.avatars}>
            {event.participants.slice(0, 4).map((person, index) => (
              <UserAvatar key={person.id} uri={person.avatarUri} initials={person.initials} color={person.avatarColor} size={28} radius={14} style={{ marginLeft: index === 0 ? 0 : -8, borderWidth: 2, borderColor: palette.surface }} />
            ))}
          </View>
